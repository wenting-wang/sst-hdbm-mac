...
# UPDATED for new HDBM (Additive mechanism)
HDBM_PARAM_RANGE = {
    "alpha_go": (0.0, 1.0),
    "alpha_stop": (0.0, 1.0),
    "k_go": (0.0, 10.0),  # Adjust max range if k_go tends to be larger
    "rho": (0.0, 1.0)
}
...
def simulate_single_dataset(args) -> Tuple[np.ndarray, np.ndarray]:
    ...
    # UPDATED: Initialize HDBM with new parameters and additive fusion
    hdbm = HDBM(
...
class AmortizedInferenceNet(nn.Module):
    def __init__(self, trial_feature_dim=5, d_model=64, n_heads=4, n_layers=2, param_dim=10):
        ...
        # flow context needs to match param_dim
...
def evaluate_parameter_recovery(model, device, num_test=16):
    ...
    # 2. Generate Scatter Plots (UPDATED: Dynamic grid size for 10 parameters)
...
def train_pipeline(n_samples=50000, epochs=200, batch_size=256, patience=30):
    ...
    dataset_file = BASE_DIR / f"e2e_dataset_{n_samples}_additive.pt"
    ...
    # UPDATED: Dynamic param_dim based on FREE_PARAM list length (now 10)
