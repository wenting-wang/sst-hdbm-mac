...
def get_subject_params(params_file, subject_id, year='baseline'):
    ...
    df['clean_id'] = df['subject_id'].astype(str).str.replace('NDAR_', '', regex=False)
    clean_target = str(subject_id).replace('NDAR_', '')
    
    year_col = 'subject_year' if 'subject_year' in df.columns else 'year'
    if year_col in df.columns:
        df[year_col] = df[year_col].astype(str)
        mask = (df["clean_id"] == clean_target) & (df[year_col] == str(year))
    else:
        mask = (df["clean_id"] == clean_target)
...
def main():
    ...
    params_csv = out_dir / f"params_posteriors_{MODEL_TAG}.csv"
    ppc_metrics_csv = out_dir / f"ppc_metrics_{MODEL_TAG}.csv" 
    output_image = out_dir / f"fig_ppc_reps_{MODEL_TAG}.png"

    try:
        subjects = select_representative_subjects(ppc_metrics_csv, args.filter_csv)
        plot_model_fit(subjects, Path(args.sst_folder), params_csv, output_image)
