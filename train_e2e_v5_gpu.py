...
# ADDITIVE HDBM + POSITIONAL ENCODING + FAST EVALUATION (POMDP FROZEN)
...
class AmortizedInferenceNet(nn.Module):
    def __init__(self, trial_feature_dim=5, d_model=64, n_heads=4, n_layers=2, param_dim=4):
        super().__init__()
...
def evaluate_parameter_recovery(model, device, num_test=32): # 测试增加到32，因为图变小了出得更快
    print("\n" + "="*50)
    ...
    plt.tight_layout()
    plot_out_path = BASE_DIR / "parameter_recovery_scatter_HDBM_ONLY.png"
    plt.savefig(plot_out_path, dpi=300)
    plt.close()
    print(f"Saved scatter plot matrix to '{plot_out_path}'.")

# --- 6. MAIN TRAINING PIPELINE ---
def train_pipeline(n_samples=2000, epochs=300, batch_size=128, patience=40):
    ...
    model_out_path = BASE_DIR / "amortized_inference_net_HDBM_ONLY.pth"
    torch.save(model.state_dict(), model_out_path)
    
    evaluate_parameter_recovery(model, device, num_test=32)

if __name__ == "__main__":
    train_pipeline(n_samples=2000, epochs=300, batch_size=128, patience=40)