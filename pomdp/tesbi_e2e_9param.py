...
9-Parameter End-to-End TeSBI Pipeline.
- Freed 3 additional parameters: cost_time, cost_go_error, cost_go_missing (Total 9).
...
PARAM_RANGES = {
    ...
    "inv_temp":        (10.0, 100.0),
    "cost_time":       (0.0001, 0.05),  # Freed
    "cost_go_error":   (0.01, 2.0),     # Freed
    "cost_go_missing": (0.01, 2.0)      # Freed
}

LINEAR_PARAMS = ["q_d_n", "q_d", "q_s_n", "q_s"]
LOG_PARAMS = ["inv_temp", "cost_stop_error", "cost_time", "cost_go_error", "cost_go_missing"]
...
DATASET_PATH = ART_DIR / "simulated_dataset_9p.pt"
MODEL_PATH = ART_DIR / "amortized_inference_net_9p.pth"
RECOVERY_CSV = ART_DIR / "params_recovery_9p.csv"
POST_SUMMARY_CSV = ART_DIR / "params_posteriors_9p.csv"
...
class EndToEndTeSBI(nn.Module):
    def __init__(self, in_dim=18, d_model=128, n_heads=8, n_layers=4, param_dim=9):
        super().__init__()
