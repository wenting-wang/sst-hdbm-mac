...
# FULL 10-PARAMETER E2E MODEL (4 HDBM + 6 POMDP) + FAST EVALUATION & REAL DATA INFERENCE
...
# Define free parameters for HDBM (Added scale_lambda)
if FUSION_MODE in ['additive_1', 'additive_2']:
    HDBM_PARAM_RANGE = {
        "eta": (0.0, 10.0),   
        "rho": (0.0, 1.0),
        "k": (0.5, 5.0),
        "scale_lambda": (1.0, 15.0)
    }
...
class AmortizedInferenceNet(nn.Module):
    def __init__(self, trial_feature_dim=18, d_model=64, n_heads=4, n_layers=2, param_dim=10):
        super().__init__()
...
def run_real_data_inference(model, device):
    ...
    out_csv_path = BASE_DIR / f"est_param_{FUSION_MODE}_finetune.csv"
