...
def enhance_features(x: torch.Tensor) -> torch.Tensor:
    """
    On-the-fly feature expansion from 5 dims (E2E original) to 18 dims (TeSBI style).
    Original 5 dims: [is_stop, ssd, go_dir, choice_idx_scaled, rt_scaled]
    """
    B, seq_len, _ = x.shape
    device = x.device
    ...
    # --- 2. Build current trial features ---
    # One-hot outcome (5 dims)
    oh = torch.nn.functional.one_hot(choice_idx, num_classes=5).float() 

    # Missing flags (1 dim each)
    rt_nan = (rt <= 0.0).float().unsqueeze(-1)
    ssd_nan = (ssd < 0.0).float().unsqueeze(-1)

    # Clean missing values to strictly 0.0
    rt_clean = rt.clone()
    rt_clean[rt <= 0.0] = 0.0
    rt_clean = rt_clean.unsqueeze(-1)

    ssd_clean = ssd.clone()
    ssd_clean[ssd < 0.0] = 0.0
    ssd_clean = ssd_clean.unsqueeze(-1)

    # --- 3. Build Lag-1 historical features ---
    prev_oh = torch.roll(oh, shifts=1, dims=1)
    prev_oh[:, 0, :] = 0.0 # Zero out first trial lag

    prev_rt = torch.roll(rt_clean, shifts=1, dims=1)
    prev_rt[:, 0, :] = 0.0

    prev_ssd = torch.roll(ssd_clean, shifts=1, dims=1)
    prev_ssd[:, 0, :] = 0.0

    delta_ssd = ssd_clean - prev_ssd

    # --- 4. Normalized Trial Index [-1.0, 1.0] ---
    t_idx = torch.linspace(-1.0, 1.0, seq_len, device=device).unsqueeze(0).unsqueeze(-1)
    t_idx = t_idx.expand(B, -1, -1) 

    # Combine all into 18 dims (5 + 1 + 1 + 1 + 1 + 5 + 1 + 1 + 1 + 1)
    # Note: 'is_stop' and 'go_dir' are dropped to strictly match TeSBI, 
    # as 'is_stop' is implicitly captured by 'ssd_nan'.
    enhanced_x = torch.cat([
        oh, rt_clean, ssd_clean, rt_nan, ssd_nan,
        prev_oh, prev_rt, prev_ssd, delta_ssd, t_idx
    ], dim=-1)
...
class AmortizedInferenceNet(nn.Module):
    def __init__(self, trial_feature_dim=18, d_model=64, n_heads=4, n_layers=2, param_dim=8):
        super().__init__()
        ...
        # NEW: Context Head to stabilize embeddings before passing to Flow
        ...
        # NEW: Switched from MAF to NSF (Neural Spline Flow)
...
def evaluate_parameter_recovery(model, device, num_test=32): 
    ...
    raw_X_test = torch.tensor(np.array(X_test_data), dtype=torch.float32).to(device)
    X_test_tensor = enhance_features(raw_X_test)
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        for i, zpath in enumerate(zip_files):
            ...
            try:
                ...
                raw_tensor_batch = tensor_seq.unsqueeze(0).to(device)
                tensor_batch = enhance_features(raw_tensor_batch)
...
def train_pipeline(epochs=300, batch_size=128, patience=40):
    ...
    model = AmortizedInferenceNet(trial_feature_dim=18, param_dim=len(FREE_PARAM)).to(device)
    ...
    for epoch in range(epochs):
        ...
        for batch_x, batch_y in train_loader:
            ...
            batch_x = enhance_features(batch_x)
            optimizer.zero_grad()
...
if __name__ == "__main__":
    ...
    if EXECUTION_MODE == 'ALL':
        ...
        trained_model = AmortizedInferenceNet(trial_feature_dim=18, param_dim=len(FREE_PARAM)).to(comp_device)
