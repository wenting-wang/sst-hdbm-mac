...
def simulate_behavior(seq_int, pomdp_p, hdbm_p=None, fusion_mode='additive_2'):
    ...
    if hdbm_p is None:
        ...
        for t in range(total_trials):
            ...
            if is_stop == 0 and res_str in ['GS', 'GE']: sim_rts[t] = rt * 40.0 
            if is_stop == 1:
...
# --- Helper function for recording data ---
def append_plot_data(data_list, arch, sub_id, metric, model_name, x_vals, mean_vals, std_vals=None):
    if std_vals is None:
        std_vals = [0.0] * len(x_vals) # Real data has no simulation variance
...
def main():
    ...
    export_data_list = [] # Container for the CSV export
    ...
    for row_idx, (title, subj_data) in enumerate(archetypes.items()):
        ...
        # Global Data (Smoothed for recording as well)
        window = 15
    ...
    print(f">>> Saved raw plotting data (Mean and Std) to {out_csv}")
