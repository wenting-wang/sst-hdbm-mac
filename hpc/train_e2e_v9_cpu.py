...
def sample_prior(n_samples: int, prior_df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Pivot to wide format so subjects are rows and parameters are columns
    ...
    # Drop any subjects that might be missing one of the required parameters
    ...
    for k in pomdp_keys:
        ...
        # Extract the specific means and stds for the sampled subjects
        means = df_mean[k].values[selected_subject_indices]
        stds = df_std[k].values[selected_subject_indices]
        
        # Add a tiny floor to std to prevent zero variance issues
        stds = np.maximum(stds, 1e-6)
        
        # Sample from the Gaussian distribution defined by the subject's specific mean and std
