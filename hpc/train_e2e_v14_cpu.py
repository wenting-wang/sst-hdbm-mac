...
# FULL 10-PARAMETER E2E MODEL (4 HDBM + 6 POMDP) WITH JOINT FINE-TUNING PRIORS
...
# Define 4 free parameters for HDBM
if FUSION_MODE in ['additive_1', 'additive_2']:
    HDBM_PARAM_RANGE = {
        "eta": (0.0, 10.0),   
        "rho": (0.0, 1.0),
        "k": (0.5, 5.0),
        "scale_lambda": (1.0, 15.0)  # [NEW] Added lambda as free parameter
    }
...
def sample_prior(n_samples: int, prior_df: pd.DataFrame) -> pd.DataFrame:
    ...
    # 1. HDBM maintains a pure uniform distribution
    for k in HDBM_PARAM_RANGE.keys():
        low, high = HDBM_PARAM_RANGE[k]
        samples[k] = np.random.uniform(low, high, n_samples)
        
    # 2. POMDP introduces historical priors
    pomdp_keys = list(POMDP_PARAM_RANGE.keys())
    
    df_filtered = prior_df[prior_df['index'].isin(pomdp_keys)]
    
    df_mean = df_filtered.pivot(index='subject_id', columns='index', values='mean')
    df_std = df_filtered.pivot(index='subject_id', columns='index', values='std')
    ...
    for k in pomdp_keys:
        ...
        stds = df_std[k].values[selected_subject_indices]
...
def simulate_single_dataset(args) -> Tuple[np.ndarray, np.ndarray]:
    ...
    # [MODIFIED] Inject all 4 HDBM parameters here
    hdbm_kwargs = {
        'alpha_go': 0.85,
        'alpha_stop': 0.85,
        'eta': params['eta'],
        'k': params['k'],
        'scale_lambda': params['scale_lambda'],  # [NEW]
...
def generate_data(n_samples=5000, part_id=1):
    print(f"\nSimulating {n_samples} FULL E2E datasets on CPU (Part {part_id})...")
    print(f"Current Fusion Mode: {FUSION_MODE.upper()}")
    print(f"Total Free Parameters: {len(FREE_PARAM)} -> {FREE_PARAM}")
    ...
    print(f"Loaded POMDP posterior data with {len(prior_df)} records.")
    
    sequences, probs = load_order_stats(ORDERS_CSV_PATH)
    
    dataset_file = BASE_DIR / f"e2e_dataset_{n_samples}_{FUSION_MODE}_part_{part_id}.pt"
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=num_workers) as executor:
        ...
        for i, future in enumerate(concurrent.futures.as_completed(futures)):
            ...
            if (i + 1) % 50 == 0:
                print(f">>> Part {part_id} Progress: [{i + 1} / {n_samples}] datasets generated...", flush=True)
    ...
    print(f"Dataset Part {part_id} successfully saved to '{dataset_file}'.")
    print("Data generation complete on CPU! Exiting script before training starts.")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Generate E2E Dataset for SST")
    parser.add_argument("--n_samples", type=int, default=5000, help="Number of samples to generate per task")
    parser.add_argument("--part_id", type=int, default=1, help="Part ID for SLURM array task")
    
    args = parser.parse_args()
    
    generate_data(n_samples=args.n_samples, part_id=args.part_id)